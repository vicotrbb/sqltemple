...
export async function initializeIpcHandlers(storage: StorageManager): Promise<void> {
  ...
  ipcMain.handle('connect-database', async (event, config: DatabaseConnectionConfig) => {
    try {
  ...
  ipcMain.handle('save-connection', async (event, connection: DatabaseConnectionConfig) => {
    try {
  ...
  // AI configuration
  ipcMain.handle('ai-set-config', async (event, config: { apiKey: string; model: string }) => {
    try {
  ...
  ipcMain.handle('ai-explain-query', async (event, sql: string) => {
    try {
      const schema = currentClient ? await currentClient.getSchemaMetadata() : undefined;
