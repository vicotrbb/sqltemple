...
contextBridge.exposeInMainWorld('api', {
  ...
  // Query history
  getQueryHistory: (connectionId?: number) => ipcRenderer.invoke('get-query-history', connectionId),
  
  // AI operations
  aiSetConfig: (config: { apiKey: string; model: string }) => ipcRenderer.invoke('ai-set-config', config),
  aiGetConfig: () => ipcRenderer.invoke('ai-get-config'),
  aiGetModels: () => ipcRenderer.invoke('ai-get-models'),
  aiAnalyzePlan: (query: string, plan: any) => ipcRenderer.invoke('ai-analyze-plan', query, plan),
  aiExplainQuery: (sql: string) => ipcRenderer.invoke('ai-explain-query', sql),
  aiCreateQuery: (prompt: string) => ipcRenderer.invoke('ai-create-query', prompt),
  aiOptimizeQuery: (sql: string) => ipcRenderer.invoke('ai-optimize-query', sql),
});