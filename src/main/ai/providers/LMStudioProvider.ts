...
export class LMStudioProvider extends BaseAIProvider {
  ...
  requiresApiKey(): boolean {
    return false; // LM Studio typically doesn't require API keys for local usage
  }
  ...
  async validateApiKey(config: AIConfig): Promise<{ isValid: boolean; error?: string }> {
    try {
      ...
      if (error.code === 'ECONNREFUSED') {
        errorMessage = 'Cannot connect to LM Studio. Make sure LM Studio is running with a local server.';
  ...
  async complete(prompt: AIPrompt, config: AIConfig): Promise<AIResponse> {
    ...
    try {
      ...
      return {
        content: data.choices?.[0]?.message?.content || "No response from LM Studio",
        usage: {
  ...
  supportsVision(): boolean {
    return false; // Most local models don't support vision
  }
