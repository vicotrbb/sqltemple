...
export class QueryHistoryService {
  async getQueryHistory(connectionId?: number, limit: number = 100): Promise<QueryHistoryServiceResult<QueryHistoryEntry[]>> {
    try {
      const result = await window.api.getQueryHistory(connectionId);
      if (result.success && result.history) {
        return { success: true, data: result.history };
      } else {
        return { success: false, error: result.error || "Failed to load query history" };
      }
    } catch (error) {
      console.error("Failed to load query history:", error);
      return { success: false, error: "An unexpected error occurred while loading query history." };
    }
  }

  async addQueryToHistory(entry: QueryHistoryEntry): Promise<QueryHistoryServiceResult<void>> {
    try {
      ...
      return { success: false, error: "An unexpected error occurred while adding query to history." };
    }
  }
