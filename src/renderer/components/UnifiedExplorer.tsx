...
type SchemaObjectType = "views" | "functions" | "procedures" | "sequences" | "triggers" | "indexes" | "domains";
...
export const UnifiedExplorer: React.FC<UnifiedExplorerProps> = ({
  ...
  const [loadedTableColumns, setLoadedTableColumns] = useState<{ [key: string]: ColumnInfo[]; }>({});
  const [loadedViewColumns, setLoadedViewColumns] = useState<{ [key: string]: ColumnInfo[]; }>({});
  const [loadingObjects, setLoadingObjects] = useState<Set<string>>(new Set());
  const [loadedSchemaObjects, setLoadedSchemaObjects] = useState<LoadedSchemaObjects>({});
  ...
  useEffect(() => {
    ...
    if (currentConnection?.id) {
      setExpandedConnections(prev => new Set(prev).add(currentConnection.id!.toString()));
    }
  }, [currentConnection]);
  ...
  const toggleSection = async (sectionKey: string, schemaName: string, objectType: SchemaObjectType) => {
    ...
    if (newExpanded.has(sectionKey)) {
      ...
      if (!loadedSchemaObjects[schemaName]?.[objectType] && !loadingObjects.has(sectionKey)) {
          ...
          switch (objectType) {
            ...
            case "functions":
              ...
              if (result.success && result.functions) {
                setLoadedSchemaObjects(prev => ({
                  ...prev,
                  [schemaName]: { ...prev[schemaName], functions: result.functions },
                }));
              }
              break;
            case "procedures":
              ...
              if (result.success && result.procedures) {
                setLoadedSchemaObjects(prev => ({
                  ...prev,
                  [schemaName]: { ...prev[schemaName], procedures: result.procedures },
                }));
              }
              break;
            case "sequences":
              ...
              if (result.success && result.sequences) {
                setLoadedSchemaObjects(prev => ({
                  ...prev,
                  [schemaName]: { ...prev[schemaName], sequences: result.sequences },
                }));
              }
              break;
            case "triggers":
              ...
              if (result.success && result.triggers) {
                setLoadedSchemaObjects(prev => ({
                  ...prev,
                  [schemaName]: { ...prev[schemaName], triggers: result.triggers },
                }));
              }
              break;
            case "indexes":
              ...
              if (result.success && result.indexes) {
                setLoadedSchemaObjects(prev => ({
                  ...prev,
                  [schemaName]: { ...prev[schemaName], indexes: result.indexes },
                }));
              }
              break;
            case "domains":
              ...
              if (result.success && result.domains) {
                setLoadedSchemaObjects(prev => ({
                  ...prev,
                  [schemaName]: { ...prev[schemaName], domains: result.domains },
                }));
              }
              break;
          }
  ...
  const toggleObject = async (objectKey: string, schemaName: string, objectName: string, objectType: "table" | "view") => {
    ...
    if (newExpanded.has(objectKey)) {
      ...
      if (
        ...
        try {
          const result = await window.api.getTableColumns(schemaName, objectName);
        ...
        try {
          const result = await window.api.getViewColumns(schemaName, objectName);
          if (result.success && result.columns) {
  ...
  const handleConnectionContextMenu = (e: React.MouseEvent, conn: DatabaseConnectionConfig) => {
    e.preventDefault();
  ...
  const handleTableContextMenu = (e: React.MouseEvent, table: TableInfo, schemaName: string) => {
    e.preventDefault();
    e.stopPropagation();
        ...
        {connections.map((conn) => {
          ...
          return (
            <ExplorerNode
              ...
              iconColor={isConnected ? "text-vscode-green" : "text-vscode-text-secondary"}
              isExpanded={isExpanded}
                ...
                <div>
                  ...
                  {schema.schemas && schema.schemas.map((schemaInfo: SchemaInfo) => (
                    <ExplorerNode
                      ...
                      level={schema.databases && schema.databases.length > 0 ? 2 : 1}
                      hasChildren={true}
                      ...
                      {expandedSchemas.has(schemaInfo.name) && (
                        <div>
                          ...
                          <ExplorerNode
                            ...
                            level={schema.databases && schema.databases.length > 0 ? 3 : 2}
                            hasChildren={true}
                            onClick={() => {
                                ...
                                {schemaInfo.tables && schemaInfo.tables.map((table: TableInfo) => {
                                  ...
                                  return (
                                    <ExplorerNode
                                      ...
                                      icon={<TableIcon size={16} />}
                                      iconColor="text-vscode-blue"
                                      count={table.columnCount !== undefined ? table.columnCount : columns.length}
                                      isExpanded={isExpanded}
                                      isLoading={isLoading}
                                      level={schema.databases && schema.databases.length > 0 ? 4 : 3}
                                      hasChildren={true}
                                      onClick={() => toggleObject(tableKey, schemaInfo.name, table.name, "table")}
                                      onContextMenu={(e) => handleTableContextMenu(e, table, schemaInfo.name)}
                                    >
                                      {isExpanded && columns.length > 0 && renderColumns(columns)}
                                      {isExpanded && columns.length === 0 && !isLoading && (
                                        <ExplorerNode
                                          id={`${tableKey}-no-columns`}
                                          label="No columns found"
                                          level={schema.databases && schema.databases.length > 0 ? 5 : 4}
                                          hasChildren={false}
                                        />
                                      )}
                                    </ExplorerNode>
                            ...
                            (view: ViewInfo, schemaName: string) => {
                              ...
                              return (
                                <ExplorerNode
                                  ...
                                  level={schema.databases && schema.databases.length > 0 ? 4 : 3}
                                  hasChildren={true}
                                  onClick={() => toggleObject(viewKey, schemaName, view.name, "view")}
                                >
                                  {isExpanded && columns.length > 0 && renderColumns(columns)}
                                  {isExpanded && columns.length === 0 && !isLoading && (
                                    <ExplorerNode
