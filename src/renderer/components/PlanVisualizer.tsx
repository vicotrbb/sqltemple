...
export const PlanVisualizer: React.FC<PlanVisualizerProps> = ({ plan, query, onClose }) => {
  ...
  if (!plan || !Array.isArray(plan) || plan.length === 0) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-vscode-text-secondary">No execution plan available</div>
      </div>
    );
  }
  ...
  const renderNode = (node: PlanNode, depth: number = 0, nodeId: string = '0'): React.ReactElement => {
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 text-sm text-vscode-text-secondary space-x-4">
              ...
              {node['Actual Total Time'] !== undefined && (
                <>
                  ...
                  <span>Actual Rows: {formatNumber(node['Actual Rows'] || 0)}</span>
                </>
              )}
            </div>
            ...
            <div className="mt-1 text-xs text-vscode-text-tertiary">
              {Object.entries(node).map(([key, value]) => {
                if (
                  ['Node Type', 'Plans', 'Total Cost', 'Plan Rows', 'Plan Width',
                   'Actual Total Time', 'Actual Rows', 'Actual Loops', 'Relation Name', 'Alias'].includes(key) ||
                  value === null ||
                  value === undefined ||
      ...
      <div className="bg-vscode-bg-secondary rounded-md shadow-2xl w-[1000px] max-h-[85vh] overflow-hidden border border-vscode-border animate-fadeIn">
        <div className="px-6 py-4 border-b border-vscode-border bg-vscode-bg-tertiary">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-lg font-medium text-vscode-text">Query Execution Plan</h2>
            <button
              onClick={onClose}
          ...
          {viewMode === 'tree' ? (
            <>
              {planData['Execution Time'] && (
                <div className="mb-4 p-3 bg-vscode-bg-tertiary rounded border border-vscode-border">
                  <div className="text-sm space-y-1">
                    <div>Planning Time: {formatTime(planData['Planning Time'])}</div>
                    <div>Execution Time: {formatTime(planData['Execution Time'])}</div>
                    <div>Total Time: {formatTime(planData['Planning Time'] + planData['Execution Time'])}</div>
                  </div>
                </div>
              )}
              ...
              <div className="mt-4 text-xs text-vscode-text-tertiary bg-vscode-bg-tertiary rounded p-3 border border-vscode-border">
                ...
                <p className="mt-1">
                  <span className="text-vscode-text-secondary">Node colors:</span> 
