import { DatabaseConnectionConfig, DatabaseSchema, SchemaInfo, TableInfo, ColumnInfo } from '../../../main/database/interfaces';

export interface SearchResult {
  id: string;
  type: 'connection' | 'database' | 'schema' | 'table' | 'view' | 'function' | 'procedure' | 'column' | 'index' | 'trigger';
  name: string;
  path: string[];
  icon: string;
  score: number;
...
export class SearchManager {
  ...
  private searchConnections(query: string, connections: DatabaseConnectionConfig[]): SearchResult[] {
    return connections
      .map(conn => ({
